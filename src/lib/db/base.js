...
export class BaseDbObject {
  ...
  async insert() {
    ...
    const [{ id }] = (await sql.query(this.insertString, row)).rows;

    console.info(chalk.dim(`[FINE] Returned id '${id}'`));
  ...
  async fetch(byField = "id", orderBy = "") {
  ...
  static async count(field = "id", filters = "") {
    console.info(
      chalk.dim(
